...
void translate(S2S_t **userT, S2D_t **userDT) {
    ...
    case SLURM:
        ...
        insert_key_double(userDT, "num_nodes", strtod(num_nodes_str, NULL));
        break;
    ...
    case PBS:
        ...
        const char *nodefile = safe_get("PBS_NODEFILE", "");
        if (strlen(nodefile) > 0) {
            num_nodes = count_unique_nodes(nodefile);
        }
...
double count_unique_nodes(const char *nodefile) {
    FILE *file = fopen(nodefile, "r");
    if (!file) return 0;

    char node[256];
    double num_nodes = 0;
    unique_node_t *nodes = NULL, *element, *tmp;

    while (fgets(node, sizeof(node), file)) {
        node[strcspn(node, "\n")] = '\0';
        HASH_FIND_STR(nodes, node, element);
        if (!element) {
            element = (unique_node_t *)malloc(sizeof(unique_node_t));
            if (element) {
                strcpy(element->node_name, node);
                HASH_ADD_STR(nodes, node_name, element);
                num_nodes++;
            }
        }
    }
    ...
    HASH_ITER(hh, nodes, element, tmp) {
        HASH_DEL(nodes, element);
        free(element);
    }
